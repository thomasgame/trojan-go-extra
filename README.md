# Trojan-Go-Extra [![Go Report Card](https://goreportcard.com/badge/github.com/thomasgame/trojan-go-extra)](https://goreportcard.com/report/github.com/thomasgame/trojan-go-extra) [![Downloads](https://img.shields.io/github/downloads/thomasgame/trojan-go-extra/total?label=downloads&logo=github&style=flat-square)](https://img.shields.io/github/downloads/thomasgame/trojan-go-extra/total?label=downloads&logo=github&style=flat-square) [![License: GPL v3](https://img.shields.io/badge/License-GPLv3-blue.svg)](LICENSE)

## 关于本仓库（Fork 说明）

| 项目 | 说明 |
|------|------|
| **本仓库** | [github.com/thomasgame/trojan-go-extra](https://github.com/thomasgame/trojan-go-extra)，由社区维护的 **Fork**，**并非**上游 [p4gefau1t/trojan-go](https://github.com/p4gefau1t/trojan-go) 的官方仓库。 |
| **上游** | 基于 PageFault 等人开发的 **Trojan-Go**；感谢原作者与贡献者。 |
| **许可证** | 与上游一致，均为 **[GNU GPL v3.0](LICENSE)**；再分发与修改须遵守 GPL（保留许可与版权声明、提供对应源码等）。详见根目录 **[NOTICE](NOTICE)**。 |
| **名称** | 「Trojan-Go」为上游项目常用名称；本仓库为独立 Fork，不代表上游立场。 |
| **联系** | 本 Fork 维护：<luring.work@gmail.com> |

**相对上游的主要改动（摘要）**

- Go 模块路径为 `github.com/thomasgame/trojan-go-extra`（见 `go.mod`）；可执行文件名为 `trojan-go-extra`。
- 增加可选 **传输层压缩**（zstd，默认开启，可配置关闭），见 [传输压缩](#传输压缩) 与 `docs/content/advance/compress.md`。
- 其余为文档、构建与仓库元数据调整。

*若将本仓库用于商业用途或大规模分发，建议咨询法律顾问。*
...
预编译二进制可执行文件可在 [Release 页面](https://github.com/thomasgame/trojan-go-extra/releases)下载。解压后即可直接运行，无其他组件依赖。
...
1. 快速启动服务端和客户端（简易模式）
    ...
    - 服务端
        ...
        ```shell
        sudo ./trojan-go-extra -server -remote 127.0.0.1:80 -local 0.0.0.0:443 -key ./your_key.key -cert ./your_cert.crt -password your_password
        ```

    - 客户端

        ```shell
        ./trojan-go-extra -client -remote example.com:443 -local 127.0.0.1:1080 -password your_password
        ```

2. 使用配置文件启动客户端 / 服务端 / 透明代理 / 中继（一般模式）

    ```shell
    ./trojan-go-extra -config config.json
    ```

3. 使用 URL 启动客户端（格式参见文档）

    ```shell
    ./trojan-go-extra -url 'trojan-go://password@cloudflare.com/?type=ws&path=%2Fpath&host=your-site.com'
    ```
...
4. 使用 Docker 部署
    ...
    docker run \
        ...
        -d \
        -v /etc/trojan-go-extra/:/etc/trojan-go-extra \
        --network host \
        thomasgame/trojan-go-extra
    ```
...
在客户端与服务器之间的链路上对 Trojan 负载进行 **zstd** 压缩，可减少跨公网传输的字节量（尤其适合文本、重复度高的流量）。**压缩默认开启**，无需在配置中写 `compress` 字段。

- **关闭压缩**：在客户端和服务端配置中**同时**加入 `compress` 段，并将 `enabled` 设为 `false`（仅当需要与未支持压缩的旧版本互通或排查问题时关闭）。
...
- **显式保持开启**（可选）：`"compress": { "enabled": true }` 与默认行为相同。
- **可选参数**：`algorithm`（当前为 zstd）、`level`、`min_size`、`buffer_size` 等说明见 [文档 - 传输压缩](docs/content/advance/compress.md)（或构建文档站点后对应章节）。

> 使用压缩时，请确保**两端**均为本分支构建且压缩开关一致；一端关闭、一端开启会导致连接异常。
...
- [Trojan-Go 上游](https://github.com/p4gefau1t/trojan-go)（本 Fork 所基于的项目）
...
[![Stargazers over time](https://starchart.cc/thomasgame/trojan-go-extra.svg)](https://starchart.cc/thomasgame/trojan-go-extra)
